...
class EventBus
{
  public:
    ...
    template <TEvent TEventFire, typename... Args> void Queue(Args&&... args);

    template <TEvent TEventFire, typename... Args> void Emit(Args&... args);
...
template <TEvent TEventFire, typename... Args> void EventBus::Queue(Args&&... args)
{
    m_Dispatcher.enqueue(TEventFire{std::forward<Args>(args)...});
}

template <TEvent TEventFire, typename... Args> void EventBus::Emit(Args&... args)
{
